...
const ControlPanel = () => {
  ...
  const { activeEditorTab: currentTab, setActiveEditorTab: setCurrentTab, editing, setEditing } = useSegmentation();
  ...
  }, [currentService.id, updateServiceSetting]);
  ...
  const getProcessAction = useCallback(() => {
    ...
    return () => execute(serviceSettings[currentService.id]);
  }, [currentService.id, activeMode, executeSmartSelect, batch.items, batch.rerunAll, execute, serviceSettings]);
